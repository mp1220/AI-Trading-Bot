...
class SystemMonitor:
    ...
    def _send_heartbeat(self, equity, cash, pos_count):
        fields = [("Equity", f"${equity:,.2f}"), ("Cash", f"${cash:,.2f}"),
                  ("Positions", str(pos_count)), ("ConnFails", str(self.connection_failures))]
        if self.latest_metrics:
            for key, value in self.latest_metrics.items():
                fields.append((key, value))
        discord_embed("🫀 System Heartbeat",
                      "All systems running normally." if not self.halted else "Bot halted.",
                      status="ok" if not self.halted else "error",
                      fields=fields, mention_role_id=NOTIFY.get("mention_role_id"))
    ...
    def halt_bot(self, reason):
        ...
        discord_embed("🚨 Threshold Triggered — Bot Halted", reason, status="error",
    ...
    def try_auto_restart(self) -> bool:
        ...
        self.connection_failures = 0
        self.last_heartbeat = 0
        discord_embed("♻️ Auto-Restart", "Threshold cooldown elapsed — resuming trading.", status="info",
                      mention_role_id=NOTIFY.get("mention_role_id"))
